...
async function _requestLiveLocation(employee, source) {
  ...
  try {
    ...
    log('INFO', 'TRACKING', 'Live location request sent via ReplyKeyboard', {
      employeeId: employee.id,
      source
    });
  } catch (error) {
    log('ERROR', 'TRACKING', 'Failed to send live location request', {
      employeeId: employee.id,
      error: error.message
    });
  }
...
app.post('/api/tracking/request/:employeeId', async (req, res) => {
  try {
    ...
    await _requestLiveLocation(employee, 'director_request');

    const label = employee.displayName || employee.name || 'Сотрудник';
    log('INFO', 'TRACKING', 'Manual tracking request sent', {
      employeeId, employeeName: label
    });
...
async function _handleLiveLocationStart(ctx, user, location) {
  ...
  await ctx.reply(
    '📡 Трансляция геолокации начата! Ваши перемещения отслеживаются.\nТелефон можно убрать — трекинг идёт в фоне.',
...
cron.schedule('* * * * *', async () => {
  try {
    ...
    for (const employee of employees) {
      ...
      try {
        await _requestLiveLocation(employee, 'work_start');
        log('INFO', 'TRACKING', 'Auto tracking request sent via ReplyKeyboard', {
          employeeId: employee.id,
          employeeName: label
        });
      } catch (error) {
